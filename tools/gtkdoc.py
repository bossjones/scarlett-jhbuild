...
class GTKDoc(object):
    ...
    def _run_command(
        self, args, env=None, cwd=None, print_output=True, ignore_warnings=False
    ):
        if print_output:
        ...
        if process.returncode != 0:
            raise Exception(
                "%s produced a non-zero return code %i" % (args[0], process.returncode)
            )

        if not ignore_warnings and ("warning" in stderr or "warning" in stdout):
            self.saw_warnings = True
            if not self._ask_yes_or_no_question(
                "%s produced warnings, " "try to continue?" % args[0]
            ):
    ...
    def _copy_doc_files_to_output_dir(self, html=True):
        if not self.doc_dir:
            self.logger.info(
                "Not copying any files from doc directory,"
        ...
        def copy_all_files_in_directory(src, dest):
            for path in os.listdir(src):
                copy_file_replacing_existing(
                    os.path.join(src, path), os.path.join(dest, path)
                )
    ...
    def _write_version_xml(self):
        ...
        if os.path.exists(version_xml_path) and os.path.exists(src_version_xml_path):
            return
    ...
    def _run_gtkdoc_scan(self):
        args = ["gtkdoc-scan", "--module=%s" % self.module_name, "--rebuild-types"]
    ...
    def _run_gtkdoc_scangobj(self):
        ...
        self._run_command(
            ["gtkdoc-scangobj", "--module=%s" % self.module_name],
            env=env,
            cwd=self.output_dir,
        )
    ...
    def _run_gtkdoc_mkhtml(self):
        ...
        if not os.path.isdir(html_dest_dir):
            raise Exception(
                "%s is not a directory, could not generate HTML" % html_dest_dir
            )
        elif not os.access(html_dest_dir, os.X_OK | os.R_OK | os.W_OK):
            raise Exception("Could not access %s to generate HTML" % html_dest_dir)

        # gtkdoc-mkhtml expects the SGML path to be absolute.
        sgml_file = os.path.join(os.path.abspath(self.output_dir), self.main_sgml_file)
        self._raise_exception_if_file_inaccessible(sgml_file)

        self._run_command(
            ["gtkdoc-mkhtml", self.module_name, sgml_file], cwd=html_dest_dir
        )

    def _run_gtkdoc_fixxref(self):
        args = ["gtkdoc-fixxref", "--module-dir=html", "--html-dir=html"]
        args.extend(
            ["--extra-dir=%s" % extra_dir for extra_dir in self.cross_reference_deps]
        )
    ...
    def rebase_installed_docs(self):
        ...
        if not os.path.isdir(html_dir):
            return
        args = ["gtkdoc-rebase", "--relative", "--html-dir=%s" % html_dir]
        args.extend(
            ["--other-dir=%s" % extra_dir for extra_dir in self.cross_reference_deps]
        )
    ...
    def api_missing_documentation(self):
        unused_doc_file = os.path.join(
            self.output_dir, self.module_name + "-unused.txt"
        )
        if not os.path.exists(unused_doc_file) or not os.access(
            unused_doc_file, os.R_OK
        ):
            return []
...
class PkgConfigGTKDoc(GTKDoc):
    ...
    def __init__(self, pkg_config_path, args):
        ...
        if not os.path.exists(pkg_config_path):
            raise Exception("Could not find pkg-config file at: %s" % pkg_config_path)

        self.cflags += " " + self._run_command(
            [pkg_config, pkg_config_path, "--cflags"], print_output=False
        )
        self.ldflags += " " + self._run_command(
            [pkg_config, pkg_config_path, "--libs"], print_output=False
        )
        self.version = self._run_command(
            [pkg_config, pkg_config_path, "--modversion"], print_output=False
        )
        self.prefix = self._run_command(
            [pkg_config, pkg_config_path, "--variable=prefix"], print_output=False
        )
